...
def ridge_plot(df_use,
               label_col,
               hist_col,
               title_loc = 0.2 ):
    
    """
    Creates a ridge (joy) plot of kernel density estimates (KDE) for a numerical column,
    grouped by categorical labels.

    Each category gets its own KDE plot stacked vertically with the category name
    displayed on the left side of each ridge.
    ...
    label_col : str
        The name of the categorical column in `df_use` used to group the data.
        Each unique value in this column will be a separate ridge in the plot.

    hist_col : str
        The name of the numerical column in `df_use` for which the KDE is computed.

    title_loc : float, optional, default=0.2
        (Currently unused) Intended to control the horizontal location of the category
        labels. Future extension for label positioning.

    Returns
    -------
    None
        Displays the ridge plot using matplotlib but does not return any values."""    
    ...
    label = df_use[label_col].unique()
    pal = sns.color_palette("nipy_spectral", len(label))
    
    # Set up the figure
    fig, axes = plt.subplots(len(label), 1, figsize=(8, 6), sharex=True)
    
    for i, (lbl, ax) in enumerate(zip(label, axes)):
        subset = df_use[df_use[label_col] == lbl]
        ...
        ax.text(
            x=subset[hist_col].min() - title_loc,  # shift left outside
            y=0,
            s=lbl,
            fontsize=11,
            weight='bold',
        ...
        ax.set_xlabel(hist_col) if i < len(label) - 1 else ax.set_xlabel(hist_col)
